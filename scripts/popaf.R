### Calculating Population Level Allele Frequencies to use in Fst Calculation ----
...
# 2.3: Individual names
mysamples=read.table("data/ebg/input/sample_names.txt")
samples=mysamples$V1

# 2.4: population info
locations=fread("data/entropy/metadata.txt", header = F)
colnames(locations)<-c("Ind","Population","Altitude","Subspecies")
pops_sp=split(locations,locations$Subspecies)

# step 3: write pop allele frequency function 
MAFbyPop_tetra<-function(ebgPL,Group,sampleIDs,SNP_names){
  ...
  cat ("Estimating af for subspecies",unique(Group$Subspecies),"\n")

  # subset to samples of the ecotype
  p12=p12[ , ,Group$Ind]
...
# step 4: run function
results <- mclapply(pops_sp,function(x) MAFbyPop_tetra(ebgpl,x,samples,pos), mc.cores = nCores)
AF_grouped=do.call(rbind,results)
write.table(AF_grouped,file="data/af/popaf/scaf_1_test_popaf.txt",sep="\t",row.names=T,quote=F)
